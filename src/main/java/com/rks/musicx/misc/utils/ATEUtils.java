...
public class ATEUtils {
    ...
    public static void setStatusBarColor(Activity activity, String key, int color) {
        try {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                ...
                switch (lightStatusMode) {
                    ...
                    case Config.LIGHT_STATUS_BAR_AUTO:
                        lightStatusEnabled = ATEUtil.isColorLight(color);
                        break;
                }
            ...
            if (Config.coloredStatusBar(activity, key))
                ((CoordinatorLayout) ((ViewGroup) activity.findViewById(android.R.id.content)).getChildAt(0)).setStatusBarBackgroundColor(getStatusBarColor(color));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
